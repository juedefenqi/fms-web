  ...
  <div id="application">
    <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="90">
      <div class="my-from-child">
        <FormItem label="注销期日 :" prop="cancelDate">
          <pickerDate
            :options="endDateOption"
            v-model="formValidate.cancelDate"
            @change="onendDateChange"
          ></pickerDate>
        </FormItem>
        <FormItem label="注销原因 :" prop="cancelText">
          <Input
            v-model="formValidate.cancelText"
            type="textarea"
...
export default {
  ...
  data() {
    return {
      ...
      ruleValidate: {
        cancelDate: [{ required: true, message: "请选择", trigger: "blur" }],
        cancelText: [
          //事项原因
  ...
  methods: {
    ...
    async extensionSave(name) {
      this.loading = true;
      console.log(name);
      requestSuccess(this, await updateCancellationApplication(name), true);
    },
    cancel(name) {
