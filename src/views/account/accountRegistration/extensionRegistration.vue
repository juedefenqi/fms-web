  ...
  <div id="application">
    <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="90">
      <div class="my-from-child">
        <FormItem label="新到期日 :" prop="extDate">
          <pickerDate
            :options="endDateOption"
            v-model="formValidate.extDate"
            @change="onendDateChange"
          ></pickerDate>
          <span style="margin-left:10px">
            <span>{{"原到期日 :"}}</span>
            <span v-if="deferOriDate">{{deferOriDate}}</span>
            <span v-else>(空)</span>
            <pickerDate
              v-show="false"
              v-model="deferOriDate"
              placeholder="原到期日"
        ...
        <FormItem label="展期原因 :" prop="extRea">
          <Input
            v-model="formValidate.extRea"
            type="textarea"
...
import { extensionSave } from "@/api/account/extension.js";
export default {
  props: {
    id: {
      type: Object,
  ...
  data() {
    return {
      ...
      ruleValidate: {
        extDate: [{ required: true, message: "请选择", trigger: "blur" }],
        extRea: [
          //事项原因
  ...
  methods: {
    ...
    async getList(val) {
      ...
      let data = requestSuccess(this, await ApplyInfo({ id: val.id }), false);
      this.deferOriDate = data.accDueDate;
      console.log(data);
      this.onstartDateChange(data.accDueDate);
    },
    async extensionSave(name) {
      this.loading = true;
      requestSuccess(this, await extensionSave(name), true);
    },
    cancel(name) {
