  ...
  <div id="application">
    <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="90">
      <div class="my-from-child">
        <FormItem label="解冻期日 :" prop="unfreezeDate">
          <pickerDate
            :options="endDateOption"
            v-model="formValidate.unfreezeDate"
            @change="onendDateChange"
          ></pickerDate>
        </FormItem>
        <FormItem label="解冻原因 :" prop="unfreezeRea">
          <Input
            v-model="formValidate.unfreezeRea"
            type="textarea"
...
import { unAccFreezeSave } from "@/api/account/accFreeze.js";
...
export default {
  ...
  data() {
    return {
      ...
      ruleValidate: {
        unfreezeDate: [{ required: true, message: "请选择", trigger: "blur" }],
        unfreezeRea: [
          //事项原因
  ...
  methods: {
    ...
    async extensionSave(name) {
      this.loading = true;
      console.log(name);
      requestSuccess(this, await unAccFreezeSave(name), true);
    },
    cancel(name) {
