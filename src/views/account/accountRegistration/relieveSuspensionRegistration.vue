  ...
  <div id="application">
    <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="90">
      <div class="my-from-child">
        <FormItem label="解除久悬日期 :" prop="unHangDate">
          <pickerDate
            :options="endDateOption"
            v-model="formValidate.unHangDate"
            @change="onendDateChange"
          ></pickerDate>
        </FormItem>
        <FormItem label="解除久悬原因 :" prop="unHangRea">
          <Input
            v-model="formValidate.unHangRea"
            type="textarea"
            :autosize="{minRows: 2,maxRows: 5}"
            placeholder="请输入解除久悬原因"
          />
        </FormItem>
      </div>
      <div class="my-footer">
...
import { unOverhangSave } from "@/api/account/overhang.js";
export default {
  props: {
    id: {
      type: Object,
  ...
  data() {
    return {
      ...
      ruleValidate: {
        unHangDate: [{ required: true, message: "请选择", trigger: "blur" }],
        unHangRea: [
          //事项原因
  ...
  methods: {
    ...
    async extensionSave(name) {
      this.loading = true;
      requestSuccess(this, await unOverhangSave(name), true);
    },
    cancel(name) {
