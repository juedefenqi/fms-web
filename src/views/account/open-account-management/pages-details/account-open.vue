<template>
  <div class="search openAccount">
    <Card>
      <Row class="add">
        <Button
          @click="$router.push({ name: 'accountApplication' })"
          type="primary"
          icon="md-add"
        >开户申请</Button>
      </Row>
...
import { statusBack, statusRouter } from "@/libs/tool.js";
import { statusApply, Approval } from "@/views/account/data/status.js";
export default {
  name: "account-open",
  components: {
    commonList,
  ...
  methods: {
    ...
    getLogList() {
      this.loading = true;
      openApply(this.searchForm).then(res => {
        this.loading = false;
        if (res.success) {
