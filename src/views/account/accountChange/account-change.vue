<template>
  <div class="search openAccount">
    <Card>
      <Row class="add">
        <Button
          @click="$router.push({ name: 'changeApplication' ,params: { id: isStore() }})"
          type="primary"
          icon="md-add"
        >变更申请</Button>
      </Row>
...
<script>
import { changeApplicationList } from "@/api/account/change.js";
...
import { changeApply ,Approval} from "@/views/account/data/status.js";
export default {
  name: "account-open",
  components: {
    commonList,
...
            let bColor = Approval(status).color;
  ...
  methods: {
    ...
    getLogList() {
      this.loading = true;
      changeApplicationList(this.searchForm).then(res => {
        this.loading = false;
        if (res.success) {
...
@import "../open-account-management/openAccountManagement.less";
</style>
