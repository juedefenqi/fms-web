<template>
  <div class="search openAccount">
    <Card>
      <Row class="add">
        <Button
          @click="$router.push({ name: 'extensionApplication' ,params: { id:isStore()}
           })"
          type="primary"
          icon="md-add"
        >展期申请</Button>
...
<script>
import { openApply } from "@/api/account/account.js";
import { accountChange } from "@/api/account/extension.js";
...
import { extensionApply, Approval } from "@/views/account/data/status.js";
export default {
  ...
  methods: {
    ...
    async getLogList() {
      this.loading = true;
      let getData = requestSuccess(
        this,
        await accountChange(this.searchForm),
        false
      );
      this.data = getData.records;
...
@import "./extension-application.less";
</style>
