<template>
  <div class="search openAccount">
    <Card>
      <Row class="add">
        <Button
          @click="$router.push({ name:'handleSuspensionApplication',params: { id:isStore()}})"
          type="primary"
          icon="md-add"
        >久悬处理申请</Button>
      </Row>
...
import { overhangApply, Approval } from "@/views/account/data/status.js";
import { overhangList } from "@/api/account/overhang.js";
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          align: "left",
          render: (h, params) => {
            let content = params.row.hangDealWay;
            return h(
              "div",
              {
                style: {}
  ...
  methods: {
    ...
    getLogList() {
      this.loading = true;
      overhangList(this.searchForm).then(res => {
        this.loading = false;
        if (res.success) {
...
@import "./application-overhang.less";
</style>
