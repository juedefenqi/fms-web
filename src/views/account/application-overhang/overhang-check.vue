            ...
            <account-card title="久悬处理申请" class="content-b-m">
              <div slot="content" class="account-content">
                <div class="cardItem">
                  ...
                  <FormItem label="处理方式 :">
                    {{formValidate.hangDealWay}}
                  </FormItem>
                  <div class="infoDetail infoTextarea">
                    <FormItem label="处理原因 :">
                      {{formValidate.hangRea}}
                    </FormItem>
                  </div>
                </div>
              </div>
...
import { overhangGet} from "@/api/account/overhang.js";
...
export default {
  ...
  methods: {
    ...
    async cancellationInfo() {
      let id = this.isStore();
      this.formValidate = requestSuccess(this, await overhangGet(id), false);
    },
  },
  mounted() {
    this.init();
